...
public class LunarSolarConverter {
    ...
    public static String getTermString(Solar solar) {
        ...
        // 二十四节气
        int convertMonth = solar.getSolarMonth() - 1;
        String termString = "";
        if (getSolarTermDay(solar.getSolarYear(), convertMonth * 2) == solar.getSolarYear()) {
            termString = LunarSolarConverter.solarTerm[convertMonth * 2];
        } else if (getSolarTermDay(solar.getSolarYear(), convertMonth * 2 + 1) == solar.getSolarYear()) {
    ...
    public synchronized static void findFestival(Solar solar, Lunar lunar) {
        ...
        for (int i = 0; i < LunarSolarConverter.sFtv.length; i++) {
            ...
            if (m.find()) {
                if (solar.getSolarMonth() == LunarSolarConverter.toInt(m.group(1)) && solar.getSolarDay() == LunarSolarConverter.toInt(m.group(2))) {
                    solar.setIsSFestival(true);
                    solar.setSolarFestivalName(m.group(4));
...
//					if ("*".equals(m.group(3))) this.isHoliday = true;
    ...
    public static Lunar SolarToLunar(Solar solar) {
        ...
        lunar.setLunarId(solar.getSolarId());
